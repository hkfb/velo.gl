 ...
 * @returns An object containing vertices and indices representing the tesselated road geometry.
 */
export function extrudePolylineToRoadOld(
  polyline: Point3D[],
  roadWidth: number
): Geometry {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    if (length === 0) {
      throw new Error(`Zero-length segment between points ${i} and ${i + 1}.`);
    }
    // Normalize direction
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === 0) {
      ...
      if (miterLen === 0) {
        ...
        // Adjust miter length to prevent excessive extension at sharp angles
        const tanHalfTheta = sinTheta / (1 + cosTheta);
        miterLength = roadWidth / 2 / tanHalfTheta;
        normal = {
          x: (miter.x / miterLen) * miterLength,
          y: (miter.y / miterLen) * miterLength,
          z: 0,
        };
      }
    }
    ...
    leftOffsets.push({
      x: polyline[i].x + normal.x,
      y: polyline[i].y + normal.y,
      z: polyline[i].z,
    });
    rightOffsets.push({
      x: polyline[i].x - normal.x,
  ...
  for (let i = 0; i < n; i++) {
    ...
    const bottomRight = { x: rightOffsets[i].x, y: rightOffsets[i].y, z: 0 };
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === 0) {
      ...
      if (miterLen === 0) {
        ...
        normal = {
          x: ((miter.x / miterLen) * (roadWidth / 2)) / miterLengthDenom,
          y: ((miter.y / miterLen) * (roadWidth / 2)) / miterLengthDenom,
          z: 0,
        };
      }
    }
